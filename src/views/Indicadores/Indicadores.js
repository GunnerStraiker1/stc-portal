...
const arrayRespuestas = ["TOTALMENTE EN DESACUERDO", "MUY EN DESACUERDO", "ALGO EN DESACUERDO", "EN DESACUERDO", "POCO EN DESACUERDO", "NI EN DESACUERDO NI DE ACUERDO", "POCO DE ACUERDO", "DE ACUERDO", "ALGO DE ACUERDO", "MUY DE ACUERDO", "TOTALMENTE DE ACUERDO"]
    ...
    indicadores.map((indi) => {
      ...
      indi.respuestas.split("#$%&").map((res, key) => {
        if (key !== indi.respuestas.split("#$%&").length - 1) {
          ...
          if(voto[key] > 0 ) dataPregunta.push({ name: res.toUpperCase(), votos: (voto[key] !== undefined || voto[key] !== null) ? parseInt(voto[key]) : 0 })
          alphabetKey++
        }
        return true;
      })
      dataPregunta.some(e => e.name === "NI EN DESACUERDO NI DE ACUERDO") ? dataPregunta = this.mapOrder(dataPregunta, arrayRespuestas, "name")  :   dataPregunta.sort((a, b) => parseInt(b.votos) - parseInt(a.votos))
