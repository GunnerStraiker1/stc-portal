...
export default class IndicadoresInfantiles extends Component {
  ...
  componentDidMount(){
    axios.get('https://stc-backend-new.appspot.com/indicadoresInfantil')
    .then(res => {
    ...
    if (e.currentTarget.id === "indicador") {
      ...
      if (e.currentTarget.id === "edo") {
        ...
        if (e.currentTarget.id === "año" && e.target.value !== 'Selecciona un año') {
          ...
          axios.get('https://stc-backend-new.appspot.com/menuIndicadoresInfantil/' + e.currentTarget.value)
          .then(res => {
                    ...
                    <Input type="select" name="select" id="indicador" onChange={this.changeSelection} disabled={this.state.statusDisabled ? true:null}>
                      <option unselectable>Selecciona un indicador</option>
                      {
                        this.state.indicadoresMenu.map((indicadorOption, key) =>{
                          return(
                            <option value={indicadorOption.indicador.toUpperCase()}>{indicadorOption.indicador.toUpperCase()}</option>
                          )
                        })
                      }
                    ...
                    this.state.preguntas.preguntas.map((pregunta, key) =>{
                      return(
                        <Col xs="12" md="12" key={key}>
                          <Card>
                            <CardHeader style={{backgroundColor: '#FFAEBA'}}><h4>{key +1 + ".- " + pregunta}</h4></CardHeader>
                            <CardBody>
